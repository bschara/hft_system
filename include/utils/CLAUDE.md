...
```
include/utils/
├── core/           macros.h (LIKELY/UNLIKELY/ASSERT/FATAL), types.h (BOOK_MAX_WIDTH/DEPTH)
├── containers/     lock_free_queue.hpp (SPSC ring buffer), memory_pool.hpp
├── config/         env_loader.hpp, csv.h, stream_config/stream_config.h
├── perf/           latency_tracker.hpp, benchmark/benchmark_utility.hpp
├── pricing/        price_utils.hpp (display only — never hot path)
└── net/            tls_client/tls_client.h, websocket/websocket.h, http_client/http_client.h
```

## LFQueue (`containers/lock_free_queue.hpp`)
...
## MemPool (`containers/memory_pool.hpp`)
...
## TLSClient (`net/tls_client/tls_client.h`)
...
## WebSocket (`net/websocket/websocket.h`)
...
## StreamConfig (`config/stream_config/stream_config.h`)
...
## Benchmark (`perf/benchmark/benchmark_utility.hpp`)
...
## LatencyTracker (`perf/latency_tracker.hpp`)
